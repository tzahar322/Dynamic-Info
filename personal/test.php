...
<nav class="nav">
    ...
    <div class="containerTest">
        <h1 class="text-center mb-4">Тест по информатике №1</h1>
        ...
        if (!isset($_SESSION['user_name'])) {
            ...
            if ($question_result->num_rows > 0) {
                ...
                if ($question_id == 6) { // Вопрос с упорядочиванием
                    ...
                    $matches = [
                        'Int()' => 'Вещественный тип',
                        'Str()' => 'Логический тип',
                        'Float()' => 'Целочисленный тип',
                        'Bool()' => 'Строковый тип'
                    ];
                    ...
                    foreach ($matches as $key => $value) {
                        ...
                        echo '<span>' . $key . '</span>';
                        echo '<select name="answers[' . $key . ']" class="form-control">';
                        echo '<option value="" disabled selected>Выберите соответствие</option>';
                        foreach ($matches as $match_value) {
                            echo '<option value="' . $match_value . '">' . $match_value . '</option>';
                        }
                        echo '</select>';
                        echo '</div>';
                    }
